...
workflow {
    ...
    mapped = ALIGN_TO_REF(adapterMarked_fastq, reference, bwa_index)
    bams = SORT_AND_INDEX_BAM(mapped)
    duplicateMarked_bams = MARK_DUPLICATES(bams)
    finalbams = MERGE_BAM_WITH_UBAM(duplicateMarked_bams, reference, seq_dict)
    gvcfs = VARIANT_CALLING(finalbams, reference, fai_index, seq_dict).collect()
    vcfs = COMBINE_AND_GENOTYPE_VCF(gvcfs, reference, fai_index, seq_dict)
    snps_and_indels = FILTER_SNPS_AND_INDELS(vcfs, reference, fai_index, seq_dict)
